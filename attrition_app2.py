...
# EXACT feature names from your model's booster
MODEL_FEATURES = [
    'Job_Level',
    'Years_at_Company',
...
# [Previous code for model loading and SHAP calculations...]
        
        # 1. Interactive Directional Bar Plot
        st.subheader("Directional Feature Importance")
        mean_shap = pd.DataFrame(shap_values, columns=input_df.columns).mean()
        ...
        fig = px.bar(
            ...
            title="How Features Affect Individual Employee Attrition Risk<br><sup>Red = Increases Risk | Blue = Decreases Risk</sup>",
            color_discrete_map={True: '#ff0051', False: "#008bfb"},  # Streamlit-style colors
            height=600
    ...
    #add line spacing
        ...
        def force_plot_html(base_value, shap_values, features):
            shap.initjs()
            plot = shap.force_plot(base_value, shap_values, features, matplotlib=False)
        ...
        if leave_prob > 50:
            ...
            top_5 = shap_df.nlargest(5, 'Impact')  # Top 5 positive SHAP values
            st.error("🚨 **High attrition risk !** ")
            st.write("**Top 5 factors increasing risk:**")
            for _, row in top_5.iterrows():
                st.write(f"- **{row['Feature']}** ➔ Increases risk by **{row['Impact']:.2f}**")
        else:
            # Low-risk case (green success)
            top_5 = shap_df.nsmallest(5, 'Impact')  # Top 5 negative SHAP values
            st.success("✅ **Low attrition risk !** ")
            st.write("**Top 5 protective factors:**") 
            for _, row in top_5.iterrows():
...
with tab3:
    st.header("Company & Department-wise Insights")
    
    if explainer is not None and 'X_test' in globals():
        # =====================================
        # 1. ORGANIZATION-LEVEL RISK FACTORS
        # =====================================
        st.subheader("Organization-level Risk Factors (All Employees)")
        st.markdown("""
        Shows average impact of features across the organization.
        """)
        ...
        risk_direction = st.radio(
            "Show factors that:",
            ["Increase Attrition (Red)", "Decrease Attrition (Blue)"],
            horizontal=True
        )
        # Filter features based on selection
        if risk_direction == "Increase Attrition (Red)":
            filtered_shap = mean_shap[mean_shap > 0].dropna()
            color = "#ff0051"
            title_suffix = "Increase Likelihood of Attrition"
        else:
            filtered_shap = mean_shap[mean_shap < 0].dropna()
            color = "#008bfb"
            title_suffix = "Decrease Likelihood of Attrition"

        sorted_features = filtered_shap.abs().sort_values(ascending=True)
        # Create the plot
        fig_global, ax_global = plt.subplots(figsize=(6, 4))
        bars = ax_global.barh(sorted_features.index,
                                sorted_features.values,
                                color=mean_shap[sorted_features.index].apply(
                                    lambda x: "#ff0051" if x > 0 else '#008bfb'))

        ax_global.set_title("Global Feature Importance\n(Red = Increase likelihood of attrition, Blue = Decrease likelihood of attrition)", pad=20)
        ax_global.set_xlabel("Mean Absolute SHAP Value (Impact Magnitude)")
        ax_global.axvline(0, color='black', linestyle='--')
        ax_global.grid(axis='x', linestyle='--', alpha=0.7)
        
        st.pyplot(fig_global)        
        # =====================================
        # 1. DEPARTMENT-SPECIFIC ANALYSIS
        # =====================================
        st.subheader("Department-Specific Analysis")
        ...
        if sum(dept_mask) > 10:  # Minimum sample threshold
            ...
            shap.plots.waterfall(
                shap.Explanation(
                    ...
                    data=employee_data[filtered_features],
